...
SCRIPT_PATH="./pacscripts/vscode-insiders-deb.pacscript"
SOURCE_BASE_URL="https://packages.microsoft.com/repos/code/pool/main/c/code-insiders/"

function get_last_version() {
    curl -s "${SOURCE_BASE_URL}" | grep -oP '(?<=code-insiders_)[^-]*' | tail -n 1
}
...
function get_file_url() {
    ...
    if [ "${2}" == '-vars' ]; then
        echo "${SOURCE_BASE_URL}code-insiders_\${pkgversion}-${hash}_\${arch}.deb"
    else
        echo "${SOURCE_BASE_URL}code-insiders_${version}-${hash}_${e_arch}.deb"
    fi
}
...
function update() {
    current_version=$(Pacscript.get_value $SCRIPT_PATH 'pkgversion')
...
function upgrade() {
    ...
    echo "name=\"vscode-insiders-deb\"" >${SCRIPT_PATH}
    {
...
upgrade)
    ...
    ;;
version)
    Pacscript.get_value $SCRIPT_PATH 'pkgversion'
    ;;
*)
    exit 1
    ;;
